...
for (let i = 0; i < runners.length; i++) {
    fullName.push({first_name: runners[i].first_name, last_name: runners[i].last_name});
}
...
runners.forEach(function(item){
  fullName.push({first_name: runners.first_name, last_name: runners.last_name});
});
...
runners.filter(function(item){
  if (item.shirt_size == "L") {
    largeShirts.push({first_name: item.first_name, last_name: item.last_name, shirt_size: item.shirt_size});
  }
});
